...
class TruckJobsReportByYear extends FPDF
{
    ...
    function header()
    {
        ...
        $this->SetXY(238, 7);
        $this->Cell(58, 7, "Jobs Per Truck Report", 0, 2, "R");
    ...
    function drawPerTruckJobCounts()
    {
        ...
        $this->addTrucksJobsPage();

        // Customer job counts for year and broken into months for chosen year
        $truck_job_counts = $database->getTruckJobCountsForFinancialYear($this->year);
   
        foreach ($truck_job_counts as $truck_job_count) {
            $this->SetXY(2, 56 + $y_adjustment);
            $this->setFillColor(255, 255, 255);
            $this->Cell($this->l_cell * 2, 5, html_entity_decode($truck_job_count['number_plate']), 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['april_count'], 0, 0, "L", 1);
            $this->Cell($this->s_cell, 5, $truck_job_count['may_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['june_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['july_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['august_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['september_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['october_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['november_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['december_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['january_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['february_count'], 0, 0, "L");
            $this->Cell($this->s_cell, 5, $truck_job_count['march_count'], 0, 0, "L");
            $this->setFillColor(230, 230, 230);
            $this->SetFont('Arial', 'B', 8);
            $this->Cell($this->s_cell, 5, $truck_job_count['total_for_year'], 0, 0, "C", 1);
            $this->SetFont('Arial', '', 8);
            // Percentage of jobs of financial year
            $percentage_of_jobs = $this->total_jobs_for_year != 0 ? number_format(($truck_job_count['total_for_year'] / $this->total_jobs_for_year) * 100, 2, '.', '') : '0.00';
...
$operator_job_report_by_year->drawPerTruckJobCounts();
$operator_job_report_by_year->SetTitle('Truck Jobs ' . $year . '-' . ($year + 1));
