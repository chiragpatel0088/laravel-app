...
class SalesPerTruckReportByYear extends FPDF
{
    ...
    function header()
    {
        ...
        $this->SetXY(238, 7);
        $this->Cell(58, 7, "Truck Sales Report", 0, 2, "R");
    ...
    function drawPerTruckSalesValues()
    {
        ...
        $this->addTruckTurnoverPage();

        // Storage for per month sales totals
        $report_sub_totals = array_fill(0, 12, 0);

        // Get all trucks to calculate sales data from
        $trucks = $database->getTrucks();
        $truck_turnover_arr = array();
        foreach ($trucks as $truck) {
            $data_arr = array();

            // Operator id
            array_push($data_arr, $truck['id']);

            // Rego
            $name = html_entity_decode($truck['number_plate']);
            array_push($data_arr, $name);

            $truck_total_turnover = 0;
            for ($i = 0; $i < 12; $i++) {
                $date = date('Y-m-d', strtotime($this->year . '-4-1' . "+" . $i . " months"));
                $monthly_turnover = $database->getJobTurnoverForMonthAndTruck($truck['id'], strtotime($date));
                $truck_total_turnover += $monthly_turnover;
                $report_sub_totals[$i] += $monthly_turnover;
                array_push($data_arr, $monthly_turnover);
            }

            array_push($data_arr, $truck_total_turnover);

            // Percentage of total turnover of financial year
            $percentage_of_total_turnover = $this->total_turnover_for_year != 0 ? number_format(($truck_total_turnover / $this->total_turnover_for_year) * 100, 2, '.', '') : '0.00';
            array_push($data_arr, $percentage_of_total_turnover);
            array_push($truck_turnover_arr, $data_arr);
        }

//        // Sort the operator data by total sales desc
//        usort($truck_turnover_arr, function ($a, $b) {
//            return $b[14] - $a[14];
//        });

        // Render all data calculated in previous loop
        foreach ($truck_turnover_arr as $truck_turnover_data) {
            $this->SetXY(2, 56 + $y_adjustment);
            $this->setFillColor(255, 255, 255);
            // Render name
            $this->Cell($this->l_cell * 2, 5, $truck_turnover_data[1], 0, 0, "L");

            for ($i = 0; $i < 12; $i++) {
                $monthly_turnover = $truck_turnover_data[$i + 2];
            ...
            $this->Cell($this->s_cell, 5, '$' . number_format($truck_turnover_data[14], 0), 0, 0, "C", 1);
            $this->SetFont('Arial', '', 8);

            // Render percentage of sales for the year
            $this->Cell($this->s_cell, 5, $truck_turnover_data[15] . "%", 0, 0, "C");
...
$operator_job_report_by_year = new SalesPerTruckReportByYear($year);
$operator_job_report_by_year->drawPerTruckSalesValues();
$operator_job_report_by_year->SetTitle('Truck Sales ' . $year . '-' . ($year + 1));
$operator_job_report_by_year->Output("I", "Jaxxon_Truck_Sales_Report.pdf");
