$(document).ready(function() {
    ...
    // If range is not empty, calculate the travel fee
    $('#modal-ready-for-invoicing').on('shown.bs.modal', function(e) {
        if ($("#invoice-range").val() != "" && $("#invoice-travel-fee").val() == "")
    ...
    $("[name=invoice-rate-type], #invoice-special-rate, #invoice-special-1, #invoice-special-2, #invoice-actual-cubics, #invoice-range, #invoice-establishment, #invoice-travel-fee, #invoice-cubic-rate, #invoice-washdown-fee, #invoice-disposal-fee, #invoice-discount, #invoice-, #invoice-hourly-rate, #invoice-actual-hours, #invoice-establishment-fee").on("change", function(e) {
        updateInvoiceTotals();
    });
