...
class Session
{
    ...
    function startSession()
    {
         ...
         */
        if (!$this->logged_in) {
            $this->username  = $_SESSION['user_login'] = GUEST_NAME;
    ...
    function checkLogin()
    {
        ...
        if (isset($_COOKIE['ediary_cookname']) && isset($_COOKIE['ediary_cookid'])) {
            $this->username = $_SESSION['user_login'] = $_COOKIE['ediary_cookname'];
            $this->userid   = $_SESSION['user_id'] = $_COOKIE['ediary_cookid'];
        }
        /* Username and userid have been set and not guest */
        if (isset($_SESSION['user_login']) && isset($_SESSION['user_id']) && $_SESSION['user_login'] != GUEST_NAME) {
            /* Confirm that username and userid are valid */
            if ($database->confirmUserID($_SESSION['user_login'], $_SESSION['user_id']) != 0) {
            ...
            $this->userinfo  = $database->getUserInfo($_SESSION['user_login']);
            $this->username  = $this->userinfo['user_login'];
    ...
    function login($subuser, $subpass, $subremember)
    {
        ...
        if ($result == 1 || $result == 2) {
            ...
            $field = "user";
            $form->setError($field, "* Your account is not active or has been disabled.");
        }
        ...
        $this->username  = $_SESSION['user_login'] = $this->userinfo['user_login'];
        $this->userid    = $_SESSION['user_id'] = $this->generateRandID();
        $this->userlevel = $this->userinfo['user_level'];
        $this->ID = $_SESSION['ID'] = $this->userinfo['ID'];

        /* Insert userid into database and update active users table */
        $database->updateUserField($this->username, "user_id", $this->userid);
        $database->addActiveUser($this->username, $this->time);
        ...
        if ($subremember) {
            ...
            setcookie("ediary_cookname", $this->username, time() + 60 * 60 * 24 * $cookie_expire, $cookie_path);
            setcookie("ediary_cookid", $this->userid, time() + 60 * 60 * 24 * $cookie_expire, $cookie_path);
        } else {
    ...
    function logout()
    {
        ...
        if (isset($_COOKIE['ediary_cookname']) && isset($_COOKIE['ediary_cookid'])) {
            ...
            setcookie("ediary_cookname", "", time() - 60 * 60 * 24 * $cookie_expire, $cookie_path);
            setcookie("ediary_cookid", "", time() - 60 * 60 * 24 * $cookie_expire, $cookie_path);
        }
     ...
     */
    function register($subemail, $subpass, $subconf_pass, $subfirstname, $sublastname, $subphone, $sublevel)
    {
        ...
        if (!$subpass) {
            ...
            /* Check if password is too long */ else if (strlen($subpass) > $config['max_pass_chars']) {
        ...
        /* No errors, add the new account to the database */ else {
            ...
            if ($database->addNewUser($subemail, $subpass, $token, $usersalt, $subfirstname, $sublastname, $subphone, $sublevel)) {
                /* Check Account activation setting and process accordingly. */

                /* E-mail Activation */
                if ($config['ACCOUNT_ACTIVATION'] == 3) {
                    $config = $database->getConfigs();
                    $mailer->sendActivation($subemail, $subemail, $subpass, $token, $config);
                    $successcode = 3;
                }
                /* Admin Activation */ else if ($config['ACCOUNT_ACTIVATION'] == 4) {
                    $config = $database->getConfigs();
                    // $mailer->adminActivation($subemail, $subemail, $subpass, $config);
                    // $mailer->activateByAdmin($subemail, $subemail, $subpass, $token, $config);
                    // $mailer->sendContact('buildaprice', 'name', 'molevomina@klipschx12.com', '123456', 'comment');
                    ...
                    $mail->Username = "james@microsolution.co.nz";
                    //Password to use for SMTP authentication
                    $mail->Password = "Bolu6191";
                    //Set who the message is to be sent from
                    $mail->setFrom('from@example.com', 'First Last');
                    //Set an alternative reply-to address
                    $mail->addReplyTo('replyto@example.com', 'First Last');
                    //Set who the message is to be sent to
                    $mail->addAddress('james@microsolution.co.nz', 'John Doe');
                ...
                /* No Activation Needed but E-mail going out */ else if ($config['EMAIL_WELCOME'] && $config['ACCOUNT_ACTIVATION'] == 5) {
                    $config = $database->getConfigs();
    ...
    function adminEditAccount($subusername, $subnewpass, $subconfnewpass, $subemail, $subuserlevel, $subusertoedit)
    {
        ...
        if ($subusername) {
            ...
            /* Check if username is already in use */ else if ($subusertoedit !== $subusername && $database->usernameTaken($subusername)) {
    ...
    function editAccount($subcurpass, $subnewpass, $subconfnewpass, $subemail)
    {
        ...
        if ($subnewpass) {
            ...
            /* Check if password is not alphanumeric */ else if (!preg_match("/^([0-9a-z])+$/i", ($subnewpass = trim($subnewpass)))) {
                $form->setError($field, "* New Password not alphanumeric, letters and numbers only!");
            }
            /* Check if passwords match */ else if ($subnewpass != $subconfnewpass) {
                $form->setError($field, "* Passwords do not match");
            }
        ...
        if ($subcurpass && $subnewpass) {
            ...
            $database->updateUserField($this->username, "user_pass", $subnewpass);
            $database->updateUserField($this->username, "user_salt", $usersalt);
        }

        /* Change Email */
        if ($subemail) {
            $database->updateUserField($this->username, "user_email", $subemail);
        }
