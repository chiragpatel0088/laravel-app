...
Editor::inst($db, 'concrete_types')
    ->fields(
        Field::inst('concrete_types.id')->set(false),
        Field::inst('concrete_types.concrete_name')
            ->validator(Validate::notEmpty(ValidateOptions::inst()
                ->message('A concrete name is required')))
            ->validator(Validate::minLen(1, ValidateOptions::inst()
                ->message('Concrete name must be at least 1 characters long')))
            ->validator(Validate::maxLen(250, ValidateOptions::inst()
                ->message('Concrete name must be no longer than 250 characters'))),
        Field::inst('concrete_types.concrete_charge')
